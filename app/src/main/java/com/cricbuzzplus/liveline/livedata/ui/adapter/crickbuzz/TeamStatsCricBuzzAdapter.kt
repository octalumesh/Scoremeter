...
import com.cricbuzzplus.liveline.livedata.ui.activity.cricbuzzs.TeamStatsDetailActivity

class TeamStatsCricBuzzAdapter(
    val list: ArrayList<TypesItem>,
    val context: Context,
    val teamId: Int
) : RecyclerView.Adapter<TeamStatsCricBuzzAdapter.MyViewHolder>() {
       ...
       val model=list[position]
        ...
        holder.binding.teamName.setOnClickListener {
            if (!model.value.isNullOrEmpty()) {
                val intent = Intent(context, TeamStatsDetailActivity::class.java)
